/interface wireguard add listen-port=13231 mtu=1420 name=wg-workshop private-key="6A3vxczDeNbiLTtznKvUlGaqAnCa/p9uUk/H1eqIzWQ="
/interface bridge add name=bridge-eoip
/interface eoip add name=eoip-workshot remote-address=10.20.1.1 tunnel-id=16
/ip pool add name=local_16 ranges=10.10.16.2-10.10.16.14
/ip dhcp-server add address-pool=local_16 interface=bridge name=dhcp_16
...
/ip address add address=10.20.1.16/24 interface=wg-workshop network=10.20.1.0
/ip address add address=10.10.16.1/28 interface=bridge network=10.10.16.0
/ip dhcp-server network add address=10.10.16.0/28 dns-server=10.20.1.1 gateway=10.10.16.1
/ip firewall mangle add action=mark-routing chain=prerouting new-routing-mark=wg disabled=yes comment="route every traffic into the tunnel" passthrough=yes src-address=10.10.16.0/28
/ip firewall address-list add address=10.10.16.0/28 list=LOCAL16
...
/system identity set name="WG_ID 16"
