/interface wireguard add listen-port=13231 mtu=1420 name=wg-workshop private-key="0JNacDaR8sIwCKwpmsupwNEJz1CaDi1xk67bGv9Cq1M="
/interface bridge add name=bridge-eoip
/interface eoip add name=eoip-workshot remote-address=10.20.1.1 tunnel-id=20
/ip pool add name=local_20 ranges=10.10.20.2-10.10.20.14
/ip dhcp-server add address-pool=local_20 interface=bridge name=dhcp_20
...
/ip address add address=10.20.1.20/24 interface=wg-workshop network=10.20.1.0
/ip address add address=10.10.20.1/28 interface=bridge network=10.10.20.0
/ip dhcp-server network add address=10.10.20.0/28 dns-server=10.20.1.1 gateway=10.10.20.1
/ip firewall mangle add action=mark-routing chain=prerouting new-routing-mark=wg disabled=yes comment="route every traffic into the tunnel" passthrough=yes src-address=10.10.20.0/28
/ip firewall address-list add address=10.10.20.0/28 list=LOCAL20
...
/system identity set name="WG_ID 20"
